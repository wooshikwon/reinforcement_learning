...
- [모방 학습 복습](#모방-학습-복습)
  - [기본 용어](#기본-용어)
  - [행동 복제 (Behavioral Cloning)](#행동-복제-behavioral-cloning)
...
- [분산 감소 (Variance Reduction)](#분산-감소-variance-reduction)
  ...
  - [분산 감소 방법](#분산-감소-방법)
  - [개선된 추정량](#개선된-추정량)
- [요약 및 과제 안내](#요약-및-과제-안내)

---

## 강의 소개

안녕하세요, GCB6206 강화학습 4주차 수업을 시작하겠습니다. 저는 오늘 강의를 맡은 교수 Youngwoon Lee입니다. 오늘은 강화학습의 가장 핵심적인 개념들을 다루게 될 예정이니, 다들 집중해서 잘 따라와 주시길 바랍니다.

오늘 강의의 정식 명칭은 **GCB6206, Reinforcement Learning** 4주차 강의입니다. 강의에 사용되는 많은 슬라이드와 과제 자료들은 스탠포드의 CS224r, UC 버클리의 CS285, 그리고 구글 딥마인드의 David Silver 강의 등 저명한 강화학습 강의들에서 많이 참고하여 만들어졌다는 점을 먼저 알려드립니다. 특히 오늘 다룰 '정책 경사도(Policy Gradient)'에 대한 내용은 Daniel Seita의 블로그 포스트도 좋은 참고 자료가 될 수 있으니 관심 있는 학생들은 한번 찾아보시길 바랍니다.

### 과제 공지

그리고 중요한 공지사항이 있습니다. 현재 진행 중인 과제에 대해 다시 한번 알려드리겠습니다.

- **HW1 Imitation Learning (모방 학습)** 과제는 **10월 12일**까지입니다. 기한이 얼마 남지 않았으니 서둘러 주시기 바랍니다.
- 그리고 오늘 배울 내용을 기반으로 한 **HW2 Policy Gradient (정책 경사도)** 과제는 **10월 26일**까지 제출해 주시면 됩니다.

과제는 모두 **PDF 리포트 형태**로 제출해야 하며, LaTeX나 다른 워드프로세싱 툴 모두 사용 가능합니다.
...
자, 그럼 먼저 지난 시간에 다뤘던 기본 용어들을 복습하면서 시작해볼까요? 이 그림은 강화학습의 전체적인 구조를 아주 잘 보여주는 그림입니다.

강화학습의 세계는 매 타임스텝 $t=0, 1, 2, ...$ 마다 '의사 결정'이 순차적으로 일어나는 과정으로 볼 수 있습니다.

- **상태 (State, $S_t$)**: 특정 시점 $t$에 에이전트가 처한 **상황**을 나타내는 모든 정보를 의미합니다. 관찰값(observation)이나 환경의 배치(configuration) 등이 될 수 있습니다.

- **정책 (Policy, $\pi(a_t|s_t)$)**: 에이전트의 **두뇌**와도 같습니다. 현재 상태 $S_t$가 주어졌을 때, 어떤 행동 $a_t$를 할지에 대한 확률 분포, 즉 **전략**을 의미합니다.

- **행동 (Action, $a_t$)**: 정책에 따라 에이전트가 실제로 수행하는 **의사 결정**입니다.

- **환경 (World, $P(s_{t+1}|s_t, a_t)$)**: 에이전트의 행동에 따라 **어떻게 반응할지**를 결정합니다. 현재 상태 $s_t$에서 에이전트가 $a_t$라는 행동을 했을 때, 다음 상태 $s_{t+1}$로 변할 확률, 즉 '전이 확률(transition probability)'로 정의됩니다.

이러한 (상태, 행동, 다음 상태)의 순차적인 흐름이 계속 반복되면서 하나의 긴 여정이 만들어지는데, 이것을 우리는 **'궤적 (Trajectory)'** 또는 **'에피소드 (Episode)'** 라고 부릅니다.

---

### 행동 복제 (Behavioral Cloning)

이러한 구조 속에서, 우리가 가장 간단하게 정책 $\pi$를 학습시키는 방법이 바로 **'모방 학습 (Imitation Learning)'**, 그 중에서도 특히 **'행동 복제 (Behavioral Cloning, BC)'**였습니다.

핵심 아이디어는 아주 간단합니다. 전문가(expert)가 특정 상황에서 어떻게 행동하는지를 데이터로 모아서, 이걸 그대로 따라 하도록 지도 학습(supervised learning)을 시키는 겁니다.

- **Data**: 전문가의 결정 데이터, 즉 '시연(demonstrations)'이라고 불리는 $(s, a)$ 쌍들의 집합 $\mathbb{D} = \{(s_0, a_0), (s_1, a_1), ...\}$ 를 사용합니다.

- **Model**: 우리는 파라미터 $\theta$를 가진 신경망 정책 $\pi_{\theta}(a|s)$를 모델로 사용합니다. 이 모델은 상태 $s$를 입력받아 행동 $a$에 대한 확률을 출력합니다.

- **Loss**: 지도 학습과 동일하게 손실 함수를 최소화하는 방향으로 $\theta$를 학습시킵니다.
  - 행동이 '분류' 문제(예: 좌회전, 우회전, 직진)라면 **교차 엔트로피 손실 (Cross-entropy loss)**을 사용하고,
  - 행동이 '회귀' 문제(예: 핸들 각도)라면 **L1 또는 L2 손실**을 사용합니다.

수식으로 표현하면, 전문가 데이터 분포 $\mathbb{D}$에서 $(s, a)$ 쌍을 샘플링했을 때, 그 상태 $s$에서 전문가가 했던 행동 $a$가 나올 로그 확률($\log \pi_{\theta}(a|s)$)을 최대화하는 것과 같습니다. 즉, 전문가의 행동을 가장 잘 모방하도록 만드는 것입니다.

---

### 모방 학습의 문제점

이렇게 간단하고 강력해 보이는 모방 학습도 몇 가지 치명적인 문제점들을 가지고 있었습니다.

#### 1. 오류 누적 (Compounding Errors)

가장 큰 문제점입니다. 학습된 정책이 작은 실수를 해서 전문가가 가보지 않은 경로로 살짝 벗어났다고 상상해 보세요. 그 생소한 상태에서는 어떻게 행동해야 할지 제대로 학습된 바가 없기 때문에 더 큰 실수를 할 확률이 높고, 그 결과 경로는 점점 더 이상적인 궤도에서 벗어나게 됩니다. 마치 눈덩이처럼 오류가 계속 커지는 것입니다.

- **해결책**: DAgger와 같은 기법들이 있습니다. 학습된 정책을 실행하다가 오류가 발생하는 지점에서 다시 전문가에게 '이 상황에선 어떻게 해야 해?'라고 물어보고, 그 데이터를 추가로 수집해서 다시 학습시키는 방식입니다.

#### 2. 다봉성 시연 데이터 (Multimodal Demonstration Data)

하나의 문제를 해결하는 좋은 방법이 여러 개 있을 때 발생하는 문제입니다. 예를 들어 로봇이 소파에서 거실 테이블로 가는 경로가 왼쪽으로 돌아가는 길, 오른쪽으로 돌아가는 길 두 가지 모두 효율적이라고 해봅시다. 모방 학습은 이 두 가지 경로를 각각 별개의 좋은 해법으로 배우는 것이 아니라, 두 경로의 '평균'적인 경로를 학습하려는 경향이 있습니다. 그 결과, 두 경로의 중간에 있는 장애물로 돌진하는, 아주 나쁜 정책이 만들어질 수 있습니다.

#### 3. 관측 불일치 (Mismatch in Observability)

데이터를 수집하는 전문가(보통 인간)는 에이전트가 접근할 수 없는 추가적인 정보나 '맥락(context)'을 알고 있는 경우가 많습니다. 대화 예시를 볼까요?

- 첫 번째 대화에서 전문가는 '주말에 농구 경기를 봤다'는 맥락을 알고 "Great, how was the basketball game last weekend?"라고 답했습니다.
- 하지만 에이전트는 "Hi, how are you?"라는 문장만 보고 저 맥락을 알 수 없습니다. 이런 데이터로 학습하면, 에이전트는 뜬금없이 농구 얘기를 하는 이상한 챗봇이 될 수 있습니다.

전문가와 에이전트가 처한 상황이 최대한 유사하도록 데이터 수집 환경을 설계하는 것이 중요합니다.

---

### 강화학습의 필요성

자, 지금까지 살펴본 것처럼 모방 학습은 간단하고 강력한 프레임워크입니다. 하지만 우리는 몇 가지 근본적인 한계에 부딪히게 됩니다.
...
자, 오늘 정말 많은 내용을 배웠습니다. 정책 경사도의 기본 원리부터 시작해서, 로그-미분 트릭을 이용한 유도 과정, REINFORCE 알고리즘, 그리고 가장 큰 문제인 높은 분산을 해결하기 위한 다양한 기법들까지 살펴보았습니다. 특히 마지막에 다룬 'Reward-to-go'와 '어드밴티지' 개념은 앞으로 배울 더 발전된 알고리즘들의 기초가 되므로 반드시 잘 이해해 두시기 바랍니다.

마지막으로 과제 공지를 다시 한번 하겠습니다.

- **HW1 Imitation Learning**: 마감일이 지났습니다. 아직 제출하지 않은 학생은 서둘러 주세요.
- **HW2 Policy Gradient**: 오늘 배운 내용을 바탕으로 한 과제입니다. 마감일은 **10월 26일**입니다.

리포트는 PDF 형식으로 제출해야 하며, LaTeX나 다른 워드 툴 모두 사용 가능합니다.

오늘 수업은 여기까지입니다. 질문 있는 학생은 남아서 질문해 주세요. 수고 많으셨습니다.
